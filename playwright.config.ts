...
export default defineConfig({
  testDir: './e2e-tests',
  /* Run tests serially to avoid browser-context crashes under the local Playwright setup */
  fullyParallel: false,
  /* Fail the build on CI if you accidentally left test.only in the source code. */
  forbidOnly: !!process.env.CI,
  /* Retry on CI only */
  retries: process.env.CI ? 2 : 0,
  /* Keep Playwright workers at one to avoid browser crashes in local and CI runs. */
  workers: 1,
